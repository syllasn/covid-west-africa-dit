...
# fonction graphe
  ...
  def graphe_sn(ct, df, date_deb ='2020-03-02' , date_fin='2020-03-20', option_1='Confirme', option_2='Contact', option_3='Importe', option_4='Communautaire',option_5='Recovered', option_6='Dead'):
    mask = (df['Date'] >= date_deb) & (df['Date'] <= date_fin)
    df_test = df.loc[mask]
    date_c = df_test[['Date','Confirmed_cases','Cum_Imported_cases','Cum_contacts','Cum_communities_cases','Cum_recored','Cum_Dead', 'Cum_evacuat_out' ]]
    #print(date_c.head())
    ...
    if  option_4 == 'Communautaire':
        trace4 = go.Scatter(
                x=date_c['Date'],
                y=date_c['Cum_communities_cases'],
                name="cas_communautaires",
                line_color='darkred',
                mode='lines+markers',
                opacity=0.8)
